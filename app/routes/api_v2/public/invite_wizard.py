...
def start_plex_auth_v2(token):
    ...
    try:
        ...
        params = {"clientID": client_id, "code": pin_code, "context[device][product]": app_name, "forwardUrl": forward_url}
...
def start_discord_auth_v2(token):
    ...
    if provided_oauth_url:
        ...
        params = {
            ...
            'state': state_token,
            'redirect_uri': Setting.get('APP_BASE_URL', request.url_root.rstrip('/')) + url_for('invites.discord_oauth_callback', _external=False),
            'prompt': 'consent',
        }
        oauth_url = f"{DISCORD_API_BASE_URL}/oauth2/authorize?{urlencode(params)}"
